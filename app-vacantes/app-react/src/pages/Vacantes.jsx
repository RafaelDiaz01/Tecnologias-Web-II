...
export default function Vacantes() {
  // Nota: `columns` y `rows` dentro del componente para poder usar `params`
  // y generar enlaces dinámicos (por ejemplo `/detalle/1`).
  const columns = [
    ...
    { field: "publicado", headerName: "Publicado", width: 150 },
    {
      ...
      renderCell: (params) => (
        <GridActionsCell {...params}>
          <GridActionsCellItem
            icon={
              <NavLink to={`/detalle/${params.id}`}>
                <Eye />
              </NavLink>
            }
            label="Ver Detalles"
          />
          <GridActionsCellItem icon={<Trash />} label="Eliminar" />
        </GridActionsCell>
      ),
    },
  ];
